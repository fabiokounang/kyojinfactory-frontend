...
import { CustomerInvoice } from '../../core/models/customer-invoice.model';
...
@Component({
  ...
  templateUrl: './customer-invoice-detail.html',
  styleUrl: './customer-invoice-detail.scss',
})
export class CustomerInvoiceDetailComponent {
  ...
  readonly inv = signal<CustomerInvoice | null>(null);
  ...
  issue(): void {
    ...
    this.busy.set(true);
    this.svc.issue(i.id).subscribe({
      next: () => { this.snackBar.open('Faktur diterbitkan', 'OK', { duration: 3000 }); this.reload(i.id); this.busy.set(false); },
  ...
  deleteInv(): void {
    ...
    this.svc.remove(i.id).subscribe({
      next: () => { this.snackBar.open('Faktur dihapus', 'OK', { duration: 3000 }); this.router.navigate(['/customer-invoices']); },
