...
@Component({
  ...
  templateUrl: './customer-invoice-list.html',
...
export class CustomerInvoiceListComponent {
  ...
  readonly rows = signal<CustomerInvoice[]>([]);
  readonly loading = signal(true);
  readonly search = signal('');
  readonly filterStatus = signal('');
  readonly filterPoId = signal<number | undefined>(undefined);
  readonly displayedColumns = ['invoiceNumber', 'invoiceDate', 'customer', 'poNumber', 'total', 'status', 'actions'];

  constructor() {
    const poId = this.route.snapshot.queryParamMap.get('customerPoId');
  ...
  deleteDraft(inv: CustomerInvoice): void {
