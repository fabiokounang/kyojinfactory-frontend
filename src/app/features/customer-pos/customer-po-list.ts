...
export class CustomerPoListComponent {
  ...
  canEditPo(po: CustomerPoSummary): boolean {
    return (
      this.isSuperAdmin() &&
      (po.status === 'DRAFT' || po.status === 'CONFIRMED' || po.status === 'IN_PRODUCTION')
    );
  }

  canDeleteDraft(po: CustomerPoSummary): boolean {
    return this.isSuperAdmin() && po.status === 'DRAFT';
  }

  remove(po: CustomerPoSummary): void {
    if (!this.canDeleteDraft(po)) return;
